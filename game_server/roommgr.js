...
function constructRoomFromDb(dbdata) {
	...
	else if (roomInfo.conf.type == "symj") {
		roomInfo.gameMgr = require("./gamemgr_symj");
	}
...
// 检查开房规则是否合法
function checkRoomRule(roomConf) {
	if (roomConf.type == "xlch" || roomConf.type == "xzdd") {
	...
	else if (roomConf.type == "symj") {
		if (roomConf.difen != null
			&& roomConf.zuidafanshu != null
			&& roomConf.jushuxuanze != null
			&& roomConf.pengkaimen != null
			&& roomConf.zfbjiegangzfbjiegang != null
			&& roomConf.yikoutingyikouting != null
			&& roomConf.zanxfgzanxfg != null
			&& roomConf.shoubayishoubayi != null
			&& roomConf.tingpaitingpai != null
			&& roomConf.guodanguodan != null
		) {
			return true
		}
	}
...
// 开房规则
function getRoomConf(roomConf, creator) {
	...
	if (roomConf.type == "xlch" || roomConf.type == "xzdd") {
		...
		conf = {
			...
			maxFan: MAX_FAN[roomConf.zuidafanshu],
			maxGames: JU_SHU[roomConf.jushuxuanze],
            pengkaimen: roomConf.pengkaimen,
            zfbjiegang: roomConf.zfbjiegang,
            yikouting: roomConf.yikouting,
            zanxfg: roomConf.zanxfg,
            shoubayi: roomConf.shoubayi,
            tingpai: roomConf.tingpai,
            guodan: roomConf.guodan,
		}
	}
}

exports.createRoom = function (creator, roomConf, gems, ip, port, callback) {
	if (!checkRoomRule(roomConf)) {
		callback(1, null);
		return;
	}
